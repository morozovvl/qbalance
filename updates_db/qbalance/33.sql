...
begin
	...
	UPDATE заявка z SET КОДВПРАЙСЕ = s.КОДВПРАЙСЕ, ИМЯВПРАЙСЕ = s.ИМЯ, ЕДИЗМВПРАЙСЕ = s.ЕДИЗМ, ЦЕНА = s.ЦЕНА, ДАТАОБРАБОТКИ = current_date, КОЛ_ОБРАБОТАННОЕ = round(КОЛ*s.КОЛ_ПРАЙС/s.КОЛ_ТОВАР), ЦЕНА_ОБРАБОТАННАЯ=s.ЦЕНА_ОБРАБОТАННАЯ, НАЛИЧИЕ = s.НАЛИЧИЕ
		FROM (SELECT p.*, p.МИНЦЕНА AS ЦЕНА_ОБРАБОТАННАЯ
			...
			) s WHERE z.КОД_ТОВАР = s.КОД_ТОВАР AND z.КОД_ДОКУМЕНТЫ129 = 0 AND ФИРМА_ПРИНУД = FALSE;	
