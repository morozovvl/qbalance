...
if (app.isSA())
{
  ...
  {
    var command = "SELECT ДАТА, SUM(ДЕБЕТ) AS ДЕБЕТ, SUM(КРЕДИТ) AS КРЕДИТ FROM (SELECT *, CASE WHEN ОПЕРНОМЕР > 0 THEN 1 ELSE 0 END AS ОПЕР FROM sp_calcobjoborot('512', 1, '" + calendar.getBeginDate().toLocaleDateString() + "', '" + calendar.getEndDate().toLocaleDateString() + "')) s GROUP BY ДАТА, ОПЕР ORDER BY ДАТА, ОПЕР;";
