...
function procPrice(num)
{
  ...
  {
    var price = 0;
    var id = reportContext.getValue("таблица.КОД", i);
    if (id > 0)
    {
//      var command = "SELECT МИНЦЕНА, КОЛ_ПРАЙС, КОЛ_ТОВАР FROM vw_прайс3 WHERE КОД_ФИРМЫ = " + firmId + " AND КОД_ТОВАР = " + id + ";";
      var command = "SELECT МИНЦЕНА, КОЛ_ПРАЙС, КОЛ_ТОВАР FROM прайс WHERE КОД_ФИРМЫ = " + firmId + " AND КОД_ТОВАР = " + id + ";";
