...
function EventAfterDeleteDocument()
{ // Событие происходит после удаления документа
// Здесь Вы можете вставить свой код
}
...
{ // Вызов этой функции происходит перед запросом к БД. Функция должна вернуть дополнительный фильтр к запросу.
	var price = table.getFormWidget().findChild("lineEditPrice").text.trim();
	if (price.length > 0)
	{
		if (filter.length > 0)
			filter = filter + " AND ";
		filter = filter + "товар.ЦЕНА" + price;
	}
	return filter;
}
