...
QSqlRecord DBFactory::getDictionariesProperties(QString tableName)
{
    QSqlRecord result;
    clearError();
    QString command = QString("SELECT * FROM %1 WHERE %2 = '%3';").arg(getObjectNameCom("vw_доступ_к_справочникам"))
            .arg(getObjectNameCom("vw_доступ_к_справочникам.таблица"))
            .arg(tableName);
...
QStringList DBFactory::getFieldsList(QString tableName)
{
    QStringList result;
    clearError();
    QSqlQuery query = execQuery(QString("SELECT * FROM %1 LIMIT 0;").arg(getObjectNameCom(tableName)));
...
{
    QString command = "SELECT DISTINCT ins.ordinal_position::integer - 1 AS \"order\", ins.column_name, ins.data_type AS type, COALESCE(ins.character_maximum_length::integer, 0) + COALESCE(ins.numeric_precision::integer, 0) AS length, COALESCE(ins.numeric_scale::integer, 0) AS \"precision\", ins.is_updatable AS updateable " \
                            "FROM information_schema.columns ins " \
                            "WHERE table_name = '%1' " \
                            "ORDER BY \"order\";";

    QSqlQuery query = execQuery(QString(command).arg(table));
...
/*
        if (QString::compare(table, getObjectName("сальдо"), Qt::CaseInsensitive) == 0)
        {
            ...
            {
                if ((QString(result->at(i).name).compare("конкол",    Qt::CaseInsensitive) == 0) ||
                    (QString(result->at(i).name).compare("концена",   Qt::CaseInsensitive) == 0) ||
                    (QString(result->at(i).name).compare("консальдо", Qt::CaseInsensitive) == 0))
                {
                      addColumnProperties(result, result->at(i).name, result->at(i).type, result->at(i).length, result->at(i).precision, result->at(i).readOnly);
                }
            }
        }
*/
...
void DBFactory::addColumnProperties(QList<FieldType>* columnsProperties, QString table, QString name, QString type, int length, int precision, bool read, int number)
{
    ...
    if (columnsProperties->count() > 0)
    {
        ...
        {
            if (columnsProperties->at(i).name == name &&
...
void DBFactory::getColumnsRestrictions(QString table, QList<FieldType>* columns)
{
    ...
    QSqlQuery query = execQuery(QString("SELECT %1, %2 " \
                                        ...
                                        "WHERE %4=5 AND "\
                                            ...
                                            "%5 ILIKE '\%*\%') AND " \
                                            "%1 ILIKE '%6.\%'").arg(getObjectNameCom("доступ.имя"))
                                                               .arg(getObjectNameCom("доступ.доступ"))
...
bool DBFactory::setTableColumnHeaderOrder(int tableId, QString column, QString header, int number, bool readOnly)
{
    QString command;
    clearError();
    command = QString("SELECT * FROM %1 WHERE %2 = %3 AND %4 = '%5';").arg(getObjectNameCom("столбцы"))
                                                                      .arg(getObjectNameCom("столбцы.код_vw_справочники_со_столбцами"))
                                                                      .arg(tableId)
                                                                      .arg(getObjectNameCom("столбцы.имя"))
                                                                      .arg(column);
    ...
    if (query.isValid())
    {
        return updateColumnHeader(tableId, column, header, number, readOnly);
    }
    return appendColumnHeader(tableId, column, header, number, readOnly);
}
...
{
    clearError();
    return exec(QString("DELETE FROM %1 WHERE %2 = %3;").arg(getObjectNameCom("топер"))
                                                             .arg(getObjectNameCom("топер.опер"))
                                                             .arg(operNumber));
}
...
bool DBFactory::addToperPrv(int operNumber, QString name, QString dbAcc, bool dbAccConst, bool dbVisible, bool dbSalVisible, QString crAcc, bool crAccConst, bool crVisible, bool crSalVisible, QString itog, bool freePrv, bool attribute)
{
    int number = 1;
    clearError();
    if (dbAcc.size() > 0 && crAcc.size() > 0)
    {
        QSqlQuery query = execQuery(QString("SELECT MAX(%1) FROM %2 WHERE %3 = %4;").arg(getObjectNameCom("топер.номер"))
...
{
    clearError();
    QString command = QString("SELECT %1, %2, %3, %4 FROM %5 WHERE trim(%6) = '%7' ORDER BY %3;").arg(getObjectNameCom("vw_столбцы.столбец"))
...
{
    QSqlQuery columnsHeaders = getColumnsHeaders(tableName);
    // Свяжем заголовки столбцов со столбцами
    if (columnsHeaders.size() > 0)
    {
        for (columnsHeaders.first(); columnsHeaders.isValid(); columnsHeaders.next())
        {
            ...
            if (number > 0)
            {
                ...
                {
                    QString column1 = columnsHeaders.record().value(getObjectName("vw_столбцы.столбец")).toString();
                    if (column1 == fields->at(i).column)
                    {
                        ...
                        if (tableName.left(16) != "СписокДокументов")
...
{
    clearError();
    QString command = QString("INSERT INTO %1 (%2, %3, %4, %5, %6) VALUES (%7, '%8', '%9', %10, %11);")
...
bool DBFactory::updateColumnHeader(int tableId, QString column, QString header, int number, bool readOnly)
{
    clearError();
    QString command;
    command = QString("UPDATE %1 SET %2 = '%3', %4 = %5, %6 = %7 WHERE %8 = %9 AND %10 = '%11';")
            .arg(getObjectNameCom("столбцы"))
            .arg(getObjectNameCom("столбцы.заголовок"))
            .arg(header)
            .arg(getObjectNameCom("столбцы.номер"))
...
void DBFactory::setFile(QString fileName, FileType type, QByteArray fileData)
{
    ...
    if (query.first())
    {
        ...
        query.clear();
        text = QString("UPDATE %1 SET %2 = (:value) WHERE %3 = '%4' AND %5 = %6;").arg(getObjectNameCom("файлы"))
    ...
    {
        query.clear();
        text = QString("INSERT INTO %1 (%2, %3, %4) VALUES ('%5', %6, (:value));").arg(getObjectNameCom("файлы"))
                                                                                  .arg(getObjectNameCom("файлы.имя"))
                                                                                  .arg(getObjectNameCom("файлы.тип"))
                                                                                  .arg(getObjectNameCom("файлы.значение"))
                                                                                  .arg(fileName)
...
QString DBFactory::getDocumentSqlSelectStatement(int oper,  Dictionaries* dictionaries, QList<ToperType>* topersList, QList<FieldType>* columnsProperties, int* retPrv1)
{
    ...
    if (topersList->count() > 0)
    {
        ...
        {   // Для всех проводок данной типовой операции
            ...
            if (prv > 0)
            {
                ...
                {// Для всех полей таблицы "проводки"
                    ...
                    for (int i = 0; i < fields.count(); i++)
                    {
                        if (fields.at(i).name == field && columnsProperties != 0)
                            addColumnProperties(columnsProperties, getObjectName("проводки"), QString("p%1__%2").arg(prv).arg(field), fields.at(i).type, fields.at(i).length, fields.at(i).precision, fields.at(i).readOnly);
                    }
                }
                if (i == 0)
                {
        ...
        {   // Если имеются атрибуты для документа в данной операции
            QString attrName = QString("атрибуты%1").arg(oper);
            getColumnsProperties(&fields, attrName);
            selectClause = "";
            foreach (QString fieldName, getFieldsList(attrName))
            {
                if (fieldName != "доккод" && fieldName != "стр")
                {
//                    selectClause.append(QString(",a.%2 AS %1__%2").arg("атрибуты").arg(fieldName));
                    selectClause.append(QString(",a.%1").arg(fieldName));
                    for (int i = 0; i < fields.count(); i++)
                        if (fields.at(i).name == fieldName && columnsProperties != 0)
//                            addColumnProperties(columnsProperties, attrName, QString("%1__%2").arg("атрибуты").arg(fieldName), fields.at(i).type, fields.at(i).length, fields.at(i).precision, (fieldName != "код") ? false : true);
                            addColumnProperties(columnsProperties, attrName, QString("%1").arg(fieldName), fields.at(i).type, fields.at(i).length, fields.at(i).precision, (fieldName != "код") ? false : true);
                }
            }
            ...
            {   // Если типовая операция включает в себя проводки
                selectStatement = QString("SELECT DISTINCT p.*%1 FROM (%2) p LEFT OUTER JOIN %3 a ON p.p1__%4=a.%4 AND p.p1__%5=a.%5").arg(selectClause)
                                                                                                                                      .arg(selectStatement)
                                                                                                                                      .arg(attrName)
                                                                                                                                      .arg(getObjectName("атрибуты.доккод"))
                                                                                                                                      .arg(getObjectName("атрибуты.стр"));
            }
            else
            {   // Если проводок нет
                selectStatement = QString("SELECT DISTINCT * FROM (SELECT a.%1 AS p1__%1%2 FROM %3 a WHERE a) p").arg(getObjectName("атрибуты.стр"))
                                                                                                                 .arg(selectClause)
                                                                                                                 .arg(attrName);
        ...
        for (int i = 0; i < topersList->count(); i++)
        {
                ...
                {   // Если в по дебетовому счету указан какой-либо справочник и этот справочник мы еще не обрабатывали
                        ...
                        foreach (QString fieldName, getFieldsList(dictName)) {
                            if (fieldName.left(4) == idFieldName + "_") {        // Если поле ссылается на другую таблицу
                                ...
                                foreach (QString setDictFieldName, getFieldsList(setDictName)) {
                                    ...
                                    for (int i = 0; i < fields.count(); i++)
                                        if (fields.at(i).name == setDictFieldName && columnsProperties != 0)
                                            addColumnProperties(columnsProperties, dictName, QString("%1__%2").arg(setDictName).arg(setDictFieldName), fields.at(i).type, fields.at(i).length, fields.at(i).precision, true);
                                }
                    ...
                    {  // Это обычный справочник
                        ...
                        foreach (QString field, getFieldsList(dictName)) {
                            ...
                            for (int i = 0; i < fields.count(); i++)
                                if (fields.at(i).name == field && columnsProperties != 0)
                                    addColumnProperties(columnsProperties, dictName, QString("%1__%2").arg(dictName).arg(field), fields.at(i).type, fields.at(i).length, fields.at(i).precision, true);
                        }
            ...
            if (topersList->at(i).dbSaldoVisible) {
                ...
                if (topersList->at(i).dbQuan) {
                    ...
                    for (int i  = 0; i < fields.count(); i++)
                        if (fields.at(i).name == field && columnsProperties != 0)
                            addColumnProperties(columnsProperties, salTableName, QString("%1__%2").arg(dictName).arg(field), fields.at(i).type, fields.at(i).length, fields.at(i).precision, true);
                ...
                for (int i  = 0; i < fields.count(); i++)
                    if (fields.at(i).name == field && columnsProperties != 0)
                        addColumnProperties(columnsProperties, salTableName, QString("%1__%2").arg(dictName).arg(field), fields.at(i).type, fields.at(i).length, fields.at(i).precision, true);
                fromClause.append(QString(" LEFT OUTER JOIN %1 %2 ON p.p%3__%4=%2.%5 AND p.p%3__%6=%2.%7").arg(getObjectNameCom("сальдо")).arg(dictName).arg(prv).arg(getObjectName("проводки.дбсчет")).arg(getObjectNameCom("сальдо.счет")).arg(getObjectName("проводки.дбкод")).arg(getObjectNameCom("сальдо.код")));
            }
            if (topersList->at(i).crSaldoVisible) {
                ...
                for (int i  = 0; i < fields.count(); i++)
                    if (fields.at(i).name == field && columnsProperties != 0)
                        addColumnProperties(columnsProperties, salTableName, QString("%1__%2").arg(dictName).arg(field), fields.at(i).type, fields.at(i).length, fields.at(i).precision, true);
                fromClause.append(QString(" LEFT OUTER JOIN %1 %2 ON p.p%3__%4=%2.%5 AND p.p%3__%6=%2.%7").arg(getObjectNameCom("сальдо")).arg(dictName).arg(prv).arg(getObjectName("проводки.крсчет")).arg(getObjectNameCom("сальдо.счет")).arg(getObjectName("проводки.кркод")).arg(getObjectNameCom("сальдо.код")));
            }
        }

        if (retPrv1 != 0)
            *retPrv1 = prv1;
        selectStatement = selectClause + fromClause + QString(" ORDER BY p1__%1 ASC").arg(getObjectName("проводки.стр"));
...
{
    clearError();
    QString command = QString("SELECT * FROM %1 WHERE trim(%2) = '%3';").arg(getObjectNameCom("vw_счета"))
                                                                        .arg(getObjectNameCom("vw_счета.счет"))
                                                                        .arg(cAcc.trimmed());
    QSqlQuery query = execQuery(command);
    query.first();
    return query.record();
...
void DBFactory::setToperPermition(int operNumber, QString user, bool menu) {
    ...
    {
        command = QString("INSERT INTO %1 (%2, %3, %4, %5) VALUES (%1, '%2', '%3', %4);").arg(getObjectNameCom("доступ"))
                                                                                         .arg(getObjectNameCom("доступ.код_типыобъектов"))
                                                                                         .arg(getObjectNameCom("доступ.имя"))
                                                                                         .arg(getObjectNameCom("доступ.пользователь"))
                                                                                         .arg(getObjectNameCom("доступ.меню"))
