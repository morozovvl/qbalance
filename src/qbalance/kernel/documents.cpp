...
QString Documents::getDocumentSqlSelectStatement(int oper,  QList<ToperType>* topersList, QList<FieldType>* columnsProperties, int* retPrv1)
{
    ...
    if (topersList->count() > 0)
    {
        ...
        int prv, prv1 = 0;
        if (columnsProperties != 0 /*nullptr*/)
        ...
        {   // Для всех проводок данной типовой операции
            ...
            if (prv > 0)
            {
                ...
                {// Для всех полей таблицы "проводки"
                    ...
                    {
                        if (fields.at(i).table == tableName && fields.at(i).name.toUpper() == field.toUpper() && columnsProperties != 0 /*nullptr*/)
        ...
        for (int i = 0; i < topersList->count(); i++)
        {
                        ...
                        foreach (QString fieldName, db->getFieldsList(dictName, 0))
                        {
                            ...
                            {        // Если поле ссылается на другую таблицу
                                ...
                                foreach (QString setDictFieldName, db->getFieldsList(setDictName, 0))
                                {
                                    ...
                                    if (!columns.contains(alias))
                                    {
                                        ...
                                        for (int i = 0; i < fields.count(); i++)
                                            if (fields.at(i).table == setDictName && fields.at(i).name.toUpper() == setDictFieldName.toUpper() && columnsProperties != 0 /*nullptr*/)
                            ...
                            else
                            {
                                ...
                                if (!columns.contains(alias))
                                {
                                    ...
                                    for (int i = 0; i < fields.count(); i++)
                                        if (fields.at(i).table == dictName && fields.at(i).name.toUpper() == fieldName.toUpper() && columnsProperties != 0 /*nullptr*/)
                    ...
                    {  // Это обычный справочник
                        ...
                        foreach (QString field, db->getFieldsList(dictName, 0))
                        {
                            ...
                            if (!columns.contains(alias))
                            {
                                ...
                                for (int i = 0; i < fields.count(); i++)
                                    if (fields.at(i).table == dictName && fields.at(i).name.toUpper() == field.toUpper() && columnsProperties != 0 /*nullptr*/)
            ...
            if (topersList->at(i).dbSaldoVisible)
            {
                ...
                if (topersList->at(i).dbQuan)
                {
                    ...
                    for (int i  = 0; i < fields.count(); i++)
                        if (fields.at(i).table == salTableName && fields.at(i).name.toUpper() == field.toUpper() && columnsProperties != 0 /*nullptr*/)
        ...
        {   // Если имеются атрибуты для документа в данной операции
            ...
            foreach (QString fieldName, db->getFieldsList(attrName, 0))
            {
                ...
                {        // Если поле ссылается на другую таблицу
                    ...
                    for (int i = 0; i < fields.count(); i++)
                        if (fields.at(i).table == attrName && fields.at(i).name.toUpper() == fieldName.toUpper() && columnsProperties != 0 /*nullptr*/)
                    ...
                    foreach (QString dictFieldName, db->getFieldsList(dictName, 0))
                    {
                        ...
                        {
                            for (int i = 0; i < fields.count(); i++)
                                if (fields.at(i).table == dictName && fields.at(i).name.toUpper() == dictFieldName.toUpper() && columnsProperties != 0 /*nullptr*/)
