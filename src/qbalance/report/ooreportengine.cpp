...
OOReportEngine::OOReportEngine(ReportScriptEngine* engine) : ReportEngine(engine)
{
}
...
bool OOReportEngine::open(QHash<QString, QVariant>* context, QString name, QString ext)
{
    ...
    if (ReportEngine::open(context, name, ext))
    {
        ...
        if (file.open())
        {
            ...
            {
                if (TApplication::exemplar()->runProcess(QString("soffice -invisible -quickstart %1 ""macro://./Standard.Main.Main(%2)""").arg(tmpFileName).arg(file.fileName()), "OpenOffice") != 0)
                    result = true;
            }
            else
            {
