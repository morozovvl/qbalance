...
bool  QtScriptShell_QAbstractMessageHandler::eventFilter(QObject*  arg__1, QEvent*  arg__2)
{
...
void QtScriptShell_QAbstractMessageHandler::handleMessage(QtMsgType  type, const QString&  description, const QUrl&  identifier, const QSourceLocation&  sourceLocation)
{
    QScriptValue _q_function = __qtscript_self.property("handleMessage");
    if (!_q_function.isFunction() || QTSCRIPT_IS_GENERATED_FUNCTION(_q_function)
        || (__qtscript_self.propertyFlags("handleMessage") & QScriptValue::QObjectMember)
        || (_q_function.data().toBool() == true)) {
        qFatal("QAbstractMessageHandler::handleMessage() is abstract!");
    } else {
        _q_function.setData(QScriptValue(true));
        QScriptEngine *_q_engine = __qtscript_self.engine();
        _q_function.call(__qtscript_self,
            ...
            << qScriptValueFromValue(_q_engine, description)
            << qScriptValueFromValue(_q_engine, identifier)
            << qScriptValueFromValue(_q_engine, sourceLocation));
        _q_function.setData(QScriptValue(false));
    }
}
